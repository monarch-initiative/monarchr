...
Monarch is a flexible KG specification:
...
* Most properties are generally listed laksj f;laskj f;lkasj f;lkASDFASFs
...
A basic visualization with [`ggraph`](https://ggraph.data-imaginist.com/):

```{r welcome_plot, eval=TRUE, echo=TRUE, fig.width=10, warning=FALSE, message=FALSE}
library(ggraph)
...
```{r file_engine_demo, eval=TRUE, echo=TRUE}
...
str(g)
```
We can also get nodes in bulk by specifying an expression over node properties. We'll use the example MONDO kg for
this example; let's first get a few nodes to see what this KG supports:

```{r mondo_nodes_explore, eval=FALSE, echo=TRUE}
...
As described by [MONDO](https://mondo.readthedocs.io/en/stable/editors-guide/rare-disease-subset/), the `subsets` column defines lists which rare-disease subsets each disease
belongs to (see the corresponding MONDO documentation for details).
