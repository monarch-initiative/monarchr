...
test_that("get_nodes file_engine works with basid id query", {
    ...
    g <- get_nodes(e, query_ids = c("MONDO:0007525", "MONDO:0007526"))
...
test_that("get_nodes file_engine works with complex query syntax", {
    filename <- system.file("tests/testthat/data", "mondo_kgx_tsv-test-10JUNE2024.tar.gz", package = "monarchr")
    e <- file_engine(filename)

    # get_nodes(id %in% c("MONDO:0007525", "MONDO:0007526")) actually does work with file_engine
    g <- e %>% get_nodes(id %in% c("MONDO:0007525", "MONDO:0007526"))
    nodes_df <- g %>% activate(nodes) %>% as.data.frame()
    expect_equal(nrow(nodes_df), 2)
    ...
    # test a big fetch
    g <- e %>% get_nodes("biolink:Disease" %in% category | "biolink:Gene" %in% category)
