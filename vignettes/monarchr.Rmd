...
Before we introduce the tools in `monarchr`, it will be helpful to have an 
overview of Knowledge Graphs (KGs), specifically those used to represent biomedical
information similar to that found in the [Monarch Initiative](https://monarchinitiative.org) KG.
...
`Cystic fibrosis`. This information is stored in a *labeled property graph*, with properties
connected to the entity nodes and the edges connecting them. The [KGX format](https://github.com/biolink/kgx/blob/master/specification/kgx-format.md) specifies a 
minimal set of properties for nodes and edges: nodes must have an `id` property and a multi-valued
`category` property, and edges must have `subject`, `predicate`, and `object` properties
indicating the source, relationship, and destination of the edge. Nodes and edges may have
other properties as well, and some of these can also be multi-valued (lists of strings, integers, etc.)

<center>![](assets/cftr_cf_edge.png){width=80%}</center>

The `monarchr` package uses `tidygraph` to represent this information, which stores node and edge
information in tables, containing list-columns for multi-valued properties:

```{r welcome, eval=TRUE, echo=FALSE}
devtools::load_all()
monarch_engine() |>
	cypher_query("MATCH (n) -[r]-> (q) RETURN n, r, q LIMIT 100 ")
