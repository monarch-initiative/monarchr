...
attached to the entities (nodes) and relationships (edges), illustrated in the following figure.

<center>![](assets/cftr_cf_edge.png){width=90%}</center>

The [KGX format](https://github.com/biolink/kgx/blob/master/specification/kgx-format.md) used by 
Monarch is simple:
...
information in tables. List columns are used for multi-valued properties. The following query finds all
of the sub-types of [Neimann-Pick Disease](https://monarchinitiative.org/MONDO:0001982), and all of the
genes associated with those.


```{r welcome, fig.width=10, warning=FALSE, message=FALSE}
...
As this example hints, `monarchr` supports different query "engines", 
fetching nodes by specified criteria, and expanding on fetched subgraphs by 
following specified relationship criteria. The primary `monarch_engine()` is backed
by the Monarch Initiative cloud-hosted Neo4j database, a generic `neo4j_engine()`
supports self-hosted Neo4j graphs, and `file_engine()` loads KGs hosted at
`https://kghub.org`.


Because `monarchr` utilizes [`tidygraph`](https://tidygraph.data-imaginist.com/) which is built on on [`igraph`](https://r.igraph.org/), it is supported by a broad ecosystem, including [packages](https://ggraph.data-imaginist.com/) for [visualization](https://datastorm-open.github.io/visNetwork/), [exploratory analyses](https://r-ega.net/), graph [munging](https://tidygraph.data-imaginist.com/reference/index.html) and [algorithms](https://r.igraph.org/reference/index.html),  [matching](https://cran.r-project.org/web/packages/iGraphMatch/vignettes/iGraphMatch.html), and [graph neural networks](https://rpubs.com/giancarlo_vercellino/spinner). This package also relies on the efficient [`neo2R`](https://cran.r-project.org/web/packages/neo2R/index.html) client for Neo4j queries, supporting 
bulk, paged results from large graphs.
...
The most accessible engine provided by `monarchr` is the `monarch_engine()`, designed to access the cloud-hosted [Monarch Initiative](https://monarchiniatiative.org) KG. No parameters needed:
...
Engines support a `fetch_nodes()` function for pulling nodes by specified criteria. The simplist is to simply provide
a set of IDs:
...
g
```
Note that *even though `fetch_nodes()` returns a graph, the result never includes edges*. It's fetch *nodes*. Edges are fetched via `expand()`, described below.

It is also possible to specify an expression describing the nodes to fetch. For example, we can fetch all of the nodes where `in_taxon_label` equals `"Homo sapiens"` using R syntax. For this example, we'll also use the `limit` feature to fetch a maximum
number of nodes.^[The default is `limit = NULL`, in which case all results are fetched, in incremental page queries in the case of Neo4j-backed engines.]
...
While the syntax does not support all R expressions (for compatibility with Neo4j), standard
comparisons `==`, `!=`, `<=`, `>=`, `<`, and `>` are, as are logical operators `&`, `|`, and `!`,
and grouping with `()`s. Since many node properties are multi-valued, we provide a special `%in_list%`
operator to allow exact match against entries.^[The `%in_list%` and other operators work the same
for `file_engine()`s. We use `%in_list%` as opposed to `%in%` because `%in%` is already defined for list
columns in R data frames, but does not provide the desired behavior. Note that `%in_list%` can only take
a length-one character vector as the left-hand side, and the right hand side must be a single
property name in the KG.]
...
Also provided is a `%~%` operator, which provides regular-expression matching against node properties (but not multi-valued
ones like `category`).
